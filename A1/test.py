...
class GenderDataset(torch.utils.data.Dataset):
    ...
    def __init__(self, imgs_dir, dataframe, transform=None):
        ...
        self.label = dataframe['gender'].map(lambda x: (x + 1) // 2).tolist()
...
#     missing_keys, unexpected_keys = model_test.load_state_dict(torch.load(r'.\googlenetauxmodel.pkl', map_location='cpu'), strict=False)
    ...
    test_dataset = GenderDataset(imgs_dir=r'Datasets\celeba_test\img', dataframe=dataframe, transform=trans)
