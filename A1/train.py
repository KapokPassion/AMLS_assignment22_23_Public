...
class GenderDataset(torch.utils.data.Dataset):
    ...
    def __init__(self, imgs_dir, dataframe, transform=None):
        ...
        self.label = dataframe['gender'].map(lambda x: (x + 1) // 2).tolist() # 0 for women, 1 for men
...
#     total_dataset = GenderDataset(imgs_dir=r'Datasets\celeba\img', dataframe=dataframe, transform=train_trans)
...
#         scheduler.step()
#         epoch_end = time.time()
#         print('Epoch cost {:2f} sec'.format(epoch_end - epoch_start))
...
def run():
    ...
    # # Use GoogLeNet for training
    # model_conv = GoogLeNet(num_classes=2, aux_logits=True)
    # # Check net
    # print(model_conv)

    # Use ResNet for training
    model_conv = ResNet34(num_classes=2)
    # Check net
    print(model_conv)
    global device
    ...
    #Define scheduler
    exp_lr_scheduler = optim.lr_scheduler.StepLR(optimizer_conv, step_size=10, gamma=0.2)
